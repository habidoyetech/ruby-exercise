...
def count_empty_seats(chart)
  ...
  chart.reduce(0) {|sum, data| sum + data.count(nil) }
...
def find_favorite(array_of_hash_objects)
  ...
  favorite = array_of_hash_objects.find { |hash| hash[:is_my_favorite?] }
  # If no hash is found, return nil
  return nil if favorite.nil?
  favorite
  # Example:
